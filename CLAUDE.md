...
## 推送信号设计（3 个套利信号，2026-07-13 封基暂停）
...
| ~~封基折价年化~~ | ~~折价>4% 且 年化>10%~~ | ~~买入持有到期~~ | ⚠️ **暂停推送** | 非确定性套利，净值波动>>安全垫 |

### 封基板块暂停推送（2026-07-13）

经豆包AI核实报告，封基折价套利**不是确定性套利**，违背项目核心定位：

1. **净值波动 >> 安全垫**：偏股基金单日波动3-5%，安全垫仅1-4%（161040 单日跌3.84%，安全垫仅1.37%）
2. **赎回费侵蚀**：0.6-0.75% 赎回费占折价13-35%
3. **流动性差**：日均100-300万，大资金无法参与
4. **资金抢筹**：折价已被市场快速抹平（7/10单日收窄2-3%）

本质是"带薄安全垫的股票投资"，不是无风险套利。

**当前状态**：
- `config.json` 的 `sealed_fund_push_enabled: false`
- `sealed_fund.py` 仍扫描+打印数据（看板可见），但不推送
- 未来若市场出现极端折价（如>15%）或找到确定性方案，设 `true` 恢复
...
5. **封基双条件阈值**：折价>4% 且 年化>10%（资金机会成本视角，2026-07-12 定稿）
   - 折价>4%：扣成本0.5%净赚3.5%+，绝对收益有意义
   - 年化>10%：满足资金机会成本（用户股票预期10-15%，低于此不如做股票）
   - 调研依据：张翼轸《封基每年白送6%》+ 零城逆影《场内折价封基指南》
   - 数学关系：年化 = 折价 / 到期年限，年化10%+必然到期近(<1年)、折价薄(4-5%)
6. **封基推送规则：首次推 + 月底提醒**（2026-07-12 定稿）
   - 首次满足阈值 -> 推 + 记录 last_pushed_month = 当前月
   - 持续满足，非月底（今天<28）-> 不推
   - 持续满足，月底（今天>=28）且当前月 != last_pushed_month -> 推月度提醒（重新验证仍满足阈值）+ 更新 last_pushed_month
   - 信号消失（折价收敛/到期）-> 清除记录
   - 数据获取失败 -> 保持现状（不误清除）
   - 再次满足 -> 重新推（新机会，首次）
   - 月度提醒会实时重算折价/年化，确保机会仍有效
7. **封基开放日自动抓取**：`base.get_fund_open_date(code)` 从东方财富 F10 页面抓取预估开放申购起始日，config.json 无需手动维护 maturity_date
8. **封基异常数据过滤**：溢价 > 20% 视为净值滞后/分红除权，跳过（如 501046 曾出现 +37.81%）
9. **时区统一北京时间**：`base.now_beijing()` 替代 `datetime.now()`（GitHub Actions 默认 UTC）

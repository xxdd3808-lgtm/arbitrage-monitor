...
5. **封基双条件阈值**：折价>3% 且 年化>10%（资金机会成本视角，2026-07-12 定稿）
   - 折价>3%：扣成本0.5%净赚2.5%+，绝对收益有意义
   - 年化>10%：满足资金机会成本（用户股票预期10-15%，低于此不如做股票）
   - 调研依据：张翼轸《封基每年白送6%》+ 零城逆影《场内折价封基指南》
   - 数学关系：年化 = 折价 / 到期年限，年化10%+必然到期近(<1年)、折价薄(3-5%)
6. **封基推送规则：首次出现才推，不重复**（2026-07-12 定稿）
   - 首次满足阈值 -> 推 + 记录到 state.json
   - 持续满足 -> 不推（避免重复骚扰）
   - 信号消失（折价收敛/到期）-> 清除记录
   - 数据获取失败 -> 保持现状（不误清除）
   - 再次满足 -> 重新推（新机会）

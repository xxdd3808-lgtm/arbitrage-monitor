...
- ❌ 可转债回售套利 -> **降级为看板参考信号**。原因：①回售价不是固定103（=100+应计利息，每只不同）②连续30天条件未验证（只查当前价）③公司可能下修消除回售（美锦转债案例：7/16股东大会下修）。变量太多，非确定性套利。看板显示正股vs回售触发价供参考
...
### 2026-07-10 回售套利降级为看板参考（commit 待push）

用户验证美锦转债(127061)推送发现3个缺陷：
- 回售价不是固定103（=100+应计利息，实际约100.55）
- 连续30天条件未验证（只查当前价<触发价，实际不足20天）
- 未排除正在下修的转债（7/16股东大会下修，回售条件可能不再成立）

改进：
- convertible_bond.py：删掉回售推送逻辑，只保留到期保本推送
- app.py：加回售参考显示（正股vs回售触发价+距回售差价），标注"不推送"
- 推送信号从7个减为6个

### 2026-07-10 新增要约收购+回售套利，吸收合并降级（commit `62982b8`）
